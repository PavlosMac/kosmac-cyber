...
This is essentially because combineLatest is an observable operator and therefore async. Signals can solve this problem by only running once for each source emission in a synchronous manner.

```typescript
const sourceA = signal('');
const sourceB = signal('');
...
Then acting on or modifying our source we can use `computed()` giving us that `map` operator functionality we know from `Rxjs`, but in a synchronous manner. A simpler api offers simpler syntax, also allowing novel Angular users to be productive without the need to understand the complexity of `Rxjs`.
...
This was one small nugget look at *Signals*. What I later realised was their intended use, which is to trigger change detection less often and more localised, thus improving performance. 

Signals paired with Angular's new Zoneless paradigm are offering more performant Angular applications. Zone.js is Angular's internal motor for checking when memory has changed and commencing DOM updates as a result. DOM updates are expensive and this meant that traditionally big Angular applications could become slow. Signals lead this new way of managing change detection by forcing the developer to indicate which variables in memory Angular needs to keep tabs on. Calling .set() informs consumers and makes the system reactive, telling Angular when new updates to the DOM are needed.
...
- **Zoneless** Signals fit right into Angular's new 'Zoneless' paradigm
